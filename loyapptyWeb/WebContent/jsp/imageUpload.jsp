...
<%
	...
	try {
		if( isMSIE ){
				...
				try{ 
					...
					while ( i.hasNext () ){
						...
						if ( !fi.isFormField() && fi.getSize() < (1024 * 400 + 1)){
							filename = Long.toString(System. currentTimeMillis())+"SL"+((String)session.getAttribute("login")).replaceAll("@|[.]","")+"."+FilenameUtils.getExtension(fi.getName());
							file = new File( properties.getProperty("fileBaseSave") + filename) ;
			    			fi.write( file ) ;
			    			session.setAttribute("fileServiceTempName",filename);
			...
			filename = Long.toString(System. currentTimeMillis())+"SL"+((String)session.getAttribute("login")).replaceAll("@|[.]","")+"."+FilenameUtils.getExtension(request.getHeader("X-File-Name"));
